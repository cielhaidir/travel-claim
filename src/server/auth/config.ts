  ...
  callbacks: {
    ...
    async session({ session, token }) {
      ...
      if (token && session.user) {
        ...
        let resolvedPermissions = authToken.permissions ?? {};
        if (
          authToken.id &&
          authToken.activeTenantId !== undefined &&
          !("inventory" in resolvedPermissions)
        ) {
          resolvedPermissions = await resolveScopedPermissions({
            activeTenantId: authToken.activeTenantId ?? null,
            roles,
            isRoot: authToken.isRoot ?? false,
            activeCustomRoleId: authToken.activeCustomRoleId ?? null,
          });
          authToken.permissions = resolvedPermissions;
        }
