  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Purchase order sekarang menggunakan master vendor dari CRM Organizations."
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <CrmMetricCard label="Vendor CRM" value={String(rows.length)} helper="Organization bertanda vendor" />
        <CrmMetricCard label="Vendor Aktif" value={String(rows.filter((item) => item.status === "ACTIVE").length)} helper="Siap dipakai membuat PO" />
        <CrmMetricCard label="Dengan Contact" value={String(rows.filter((item) => item.contacts.length > 0).length)} helper="Memudahkan follow up PO" />
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_1fr]">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Draft Purchase Order</h2>
              <p className="mt-1 text-sm text-gray-500">Pilih vendor langsung dari master CRM, bukan dari master terpisah.</p>
            </div>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <label className="space-y-2">
              ...
              <input value="AUTO / PO-XXXX" readOnly className={`${crmInputClassName} bg-gray-50`} />
            </label>
            <label className="space-y-2">
              <span className="text-sm font-medium text-gray-700">Tanggal PO</span>
              <input type="date" className={crmInputClassName} />
            </label>
            <label className="space-y-2 md:col-span-2">
              <span className="text-sm font-medium text-gray-700">Referensi PR</span>
              <input placeholder="Opsional, contoh: PR-2026-0012" className={crmInputClassName} />
            </label>
          </div>
          ...
          {selectedVendor ? (
            ...
            <div className="mt-5">
              <CrmEmptyHint text="Pilih vendor CRM terlebih dahulu untuk melihat preview master data yang akan dipakai di PO." />
            </div>
          )}
        ...
        <div className="space-y-6">
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            />
            <p className="mt-2 text-xs text-gray-500">Filter ini mengambil data langsung dari CRM Organizations yang ditandai sebagai vendor.</p>
          </div>

          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-gray-900">Alur Integrasi</h2>
            <ul className="mt-4 list-disc space-y-2 pl-5 text-sm text-gray-600">
              <li>Vendor dipilih dari CRM master berdasarkan flag vendor.</li>
              <li>PO akan memakai identitas vendor yang sama dengan CRM.</li>
              <li>Goods receipt dan invoice vendor nantinya mengacu ke vendor CRM yang sama.</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 px-5 py-4">
          <h2 className="text-lg font-semibold text-gray-900">Preview Vendor CRM untuk PO</h2>
