...
export function MainDashboard() {
  ...
  const financeDashboardQuery = api.dashboard.getFinanceDashboard.useQuery(
    {},
    {
      enabled: !!session?.user && canUseFinanceDashboard,
  ...
  const coaQuery = api.chartOfAccount.getAll.useQuery(
    { limit: 100 },
    {
      enabled: !!session?.user && canUseFinanceDashboard && canReadCoa,
      refetchOnWindowFocus: false,
    },
  );
  ...
  const journalQuery = api.journalEntry.list.useQuery(
    { limit: 10 },
    {
      enabled: !!session?.user && canUseFinanceDashboard && canReadJournals,
      refetchOnWindowFocus: false,
    },
  );
                  ...
                  (financeDashboard?.recentPayments ?? [])
                    ...
                    .map((payment) => (
                      <ListRow
                        ...
                        subtitle={
                          payment.travelRequest?.destination ?? "Tanpa tujuan"
                        }
            ...
            <Panel title="Aksi Cepat" description="Navigasi cepat ke modul utama">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <QuickLink
                  ...
                  description="Kontrol uang muka, piutang, dan hutang karyawan"
                />
                <QuickLink
                  href="/accounting"
                  label="Accounting"
                ...
                <QuickLink
                  href="/accounting"
                  label="Detail Akun Saldo"
                  description="Masuk ke daftar dan drill-down mutasi akun"
                />
                <QuickLink
        ...
        <Panel title="Aksi Cepat" description="Akses utama ke modul operasional">
          <div className="grid gap-3 sm:grid-cols-3">
            <QuickLink
              ...
              description="Buat dan pantau pengajuan"
            />
            <QuickLink
              href="/claims"
              label="Klaim"
              description="Lihat dan ajukan klaim"
            />
            <QuickLink
              href="/approvals"
              label="Persetujuan"
